...
Document.prototype.receive = function(message, connection){
    console.log("doc:receive "+message);
	this._broadcast({type: 'MSG', data: message}, connection);
	this._messages.push(message);
};

Document.prototype.addUser = function(message, connection){
    if(this._userNames[message.userName] != null)
        return;
    this._userNames[message.userName] = message.userName;
	...
	this._broadcast({type: 'USER_CONNECTED', data: {id: user.id, name: user.name}}, connection);
};
...
Document.prototype._broadcast = function(message, except){
    console.log("doc:_broadcast "+message);
	...
	for(var i = 0; i < this._users.length; i++){
		...
		if(user.connection != except){
            console.log("doc:_broadcast to "+user);
			user.connection.sendUTF(msg);
		}
	}
};
...
Document.prototype.removeUser = function(connection){
    ...
    if(user!=null){
        this._userNames[user.name]= null;
        this._users.splice(index, 1);
        this._broadcast({type: 'USER_DISCONNECTED', data: {id: user.id}}, connection);
    }
